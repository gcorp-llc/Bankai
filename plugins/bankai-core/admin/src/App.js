	...
	const saveModules = async ( updatedModules ) => {
		...
		try {
			...
			if ( response && response.success ) {
				setNotice( {
					status: 'success',
					message: response.message || __( 'Settings saved successfully.', 'bankai-core' ),
				} );
			} else {
				setNotice( {
					status: 'error',
					message: __( 'Failed to save settings.', 'bankai-core' ),
				} );
			}
		} catch ( err ) {
			setNotice( {
				status: 'error',
				message: err.message || __( 'An error occurred while saving settings.', 'bankai-core' ),
			} );
		} finally {
			setLoading( false );
		}
	...
	return (
		<Layout>
			<Header
				...
				subtitle={ __( 'مدیریت یکپارچه ماژول‌های سئو، سرعت، رسانه و هوش مصنوعی', 'bankai-core' ) }
			/>
